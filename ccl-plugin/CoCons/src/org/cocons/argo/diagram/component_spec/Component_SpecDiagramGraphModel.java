...
//Jan	    Collection ends = ((MTaggedValue)node).getClientDependencies();

//Jan	    ends.addAll(((MTaggedValue)node).getClientDependencies());
...
*/
	...
	if ( (fromP instanceof MClass) && (toP instanceof MInterface) ) return true;
        if ( (fromP instanceof MInterface) && (toP instanceof MClass) ) return true;
        if ( (fromP instanceof MInterface) && (toP instanceof MInterface) ) return true;
	return false;
	...
	*/
	  ...
	  if ((fromPort instanceof MClass) && (toPort instanceof MInterface)) {
		  ...
		  MAssociation asc = MMUtil.SINGLETON.buildAssociation(fromIntf, toIntf);
		  addEdge(asc);
                  return asc;
	      }
	      else {
