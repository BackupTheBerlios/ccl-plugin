...
import ru.novosoft.uml.foundation.core.MElement;import ru.novosoft.uml.foundation.core.MInterface;import ru.novosoft.uml.foundation.core.MClass;/**
...
public boolean objectMatchsType(Object possClass, String baseClassType) {
	...
	if(baseClassType == null || possClass == null) {
		return false;
	}
	...
	if(baseClassType.compareTo(BUSINESS_TYPE) == 0) {
		if(possClass instanceof MClass) {
			return "<<type>>".compareTo(((MClass) possClass).getStereotype().getBaseClass()) == 0 ||
			"<<info type>>".compareTo(((MClass) possClass).getStereotype().getBaseClass()) == 0;
		}
	}

	if(baseClassType.compareTo(COMPONENT) == 0) {
		if(possClass instanceof MClass) {
			return "<<comp spec>>".compareTo(((MClass) possClass).getStereotype().getBaseClass()) == 0;
		}
	}
